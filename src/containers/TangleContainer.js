...
const alphaaMin = 0;
const alphaaMax = 1;
const alphaaDefault = 0.01;
const alphabDefault = 0;
...
const TipAlgorithmLabel = ({ selectedAlgorithm, onChange, algoKey }) => (
  <label className="container" key={algoKey}>
    <div style={{ fontSize: 10 }}>{tipSelectionDictionary[algoKey].label}</div>
    <input
      type="radio"
      name="radio"
      value={algoKey}
...
class TangleContainer extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      alphaa: alphaaDefault,
      alphab: alphabDefault,
    ...
    this.force.on("tick", () => {
      ...
      for (let node of this.state.nodes) {
        node.y = Math.max(
          this.state.nodeRadius,
          Math.min(this.state.height - this.state.nodeRadius, node.y)
        );
      }
      ...
      () => {
        ...
        this.force
          .force(
            ...
            d3Force
              .forceCollide()
              .radius(this.state.nodeRadius * 2)
              .strength(0.01)
              .iterations(15)
          )
          .force(
          ...
          .force(
            ...
            d3Force
              ...
              .links(this.state.links)
              .strength(0.5)
              .distance(this.state.nodeRadius * 3)
          ); // strength in [0,1]
  ...
  startNewTangle() {
    ...
    const tangle = generateTangle({
      ...
      alphaa: this.state.alphaa,
      nodeRadius,
      tipSelectionAlgorithm:
        tipSelectionDictionary[this.state.tipSelectionAlgorithm].algo
    });
  ...
  render() {
    const { nodeCount, lambda, alphaa, width, height } = this.state;
    ...
    return (
      <div>
        <div className="top-bar-container" style={{ width }}>
          ...
          <div className="top-bar-row">
            <div className="slider-title">alphaa</div>
            <div className="slider-container">
              <SliderContainer
                min={alphaaMin}
                max={alphaaMax}
                step={0.001}
                defaultValue={alphaaDefault}
                value={alphaa}
                marks={{
                  [alphaaMin]: `${alphaaMin}`,
                  [alphaaMax]: `${alphaaMax}`
                ...
                onChange={alphaa => {
                  this.setState(Object.assign(this.state, { alphaa }));
                  this.startNewTangle();
                  this.startNewTangledo();
                }}
              />
        ...
        <Tangle
          ...
          showLabels={
            this.state.nodeRadius > showLabelsMinimumRadius ? true : false
          }
        />
      </div>
    );
