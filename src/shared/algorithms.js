...
const weightedChooseb = (arr, weights) => {
  ...
  console.log("arr length is", arr[arr.length - 1]);
...
export const hybridRandomWalk = ({ nodes, links, start, alphab }) => {
  ...
  while (!isTip({ links, node: particle })) {
    ...
    const weights = normalizedWeights.map((w, index) => {
      console.log("[hybrid]", { alphabINdex: alphab[index] });
      return Math.exp(alphab[index] || 0 * w);
    });

    particle = weightedChooseb(approvers, weights, alphab);
    console.log("[hybridRandomWalk]", { cumWeights, approvers, weights });
  }
...
export const weightedRandomWalk = ({ nodes, links, start, alphaa }) => {
  let particle = start;
  console.log("[weightedRandomWalk]", alphaa);
  while (!isTip({ links, node: particle })) {
    const approvers = getApprovers({ links, node: particle });
